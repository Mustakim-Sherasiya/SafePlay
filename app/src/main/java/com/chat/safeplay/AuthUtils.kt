    ...
    auth.signInWithCredential(credential)
        .addOnCompleteListener { phoneTask ->
            if (!phoneTask.isSuccessful) {
                onFailure("OTP verification failed: ${phoneTask.exception?.message}")
                return@addOnCompleteListener
            }

            val phoneUser = auth.currentUser
            val phoneNumber = phoneUser?.phoneNumber ?: ""

            // Step 2: Link email/password with same UID
            val emailCredential = EmailAuthProvider.getCredential(email.trim(), password)
            phoneUser?.linkWithCredential(emailCredential)
                ?.addOnCompleteListener { linkTask ->
                    if (linkTask.isSuccessful) {
                        val linkedUser = linkTask.result?.user
                        val uid = linkedUser?.uid ?: return@addOnCompleteListener onFailure("User link failed")

                        // Prepare combined contact string for UI
                        val contactCombined = "${email.trim()} • $phoneNumber"

                        // Step 3: Generate publicId and save user info in Firestore
                        ensureUniquePublicIdUnique { success, publicId ->
                            if (success && publicId != null) {
                                val userData = hashMapOf(
                                    ...
                                    "emailVerified" to linkedUser.isEmailVerified
                                )

                                firestore.collection("users").document(uid)
                                    .set(userData, SetOptions.merge())
                                    .addOnSuccessListener {
                                        // Step 4: Send email verification and logout
                                ...
                                onFailure("Failed to generate publicId")
                            }
                        }
                    } else {
                        onFailure("Account link failed: ${linkTask.exception?.message}")
                    }
                }
        }
}
